...
Provides support for explaining the failures in the content of the request.
'''
...
@injected
class FailureExplainHandler(HandlerProcessor):
    '''
    Implementation for a processor that provides on the response a form of the failure that can be extracted from 
    the response code and failure message, this processor uses the code status (success) in order to trigger the failure
    response.
    '''
    
    descriptions = list
    # The descriptions (list[tuple(IVerifier, tuple(string), dictionary{string: object})]) used in constructing the failure.
    charSet = 'ASCII'
    # The content encoding to use for response.
    type = 'text/plain'
    # The content type for the reported failure.
    ...
    def process(self, chain, response:Response, responseCnt:ResponseContent, **keyargs):
        ...
        Process the failure into a response content.
        '''
        assert isinstance(response, Response), 'Invalid response %s' % response
        assert isinstance(responseCnt, ResponseContent), 'Invalid response content %s' % responseCnt

        if response.isSuccess is not False or not response.failures: return  # Not in failure.
        ...
        if response.failures:
            for messages, data, contexts in response.failures:
