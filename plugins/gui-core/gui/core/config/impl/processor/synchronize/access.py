...
import logging
from acl.api.access import IAccessService, generateId
from acl.api.filter import IFilterService
...
class SynchronizeActionHandler(HandlerProcessor):
    ...
    def __init__(self):
        assert isinstance(self.accessService, IAccessService), \
        'Invalid access service %s' % self.accessService
    ...
    def process(self, chain, solicit:Solicit, **keyargs):
        ...
        for repository in repositories:
            ...
            accessesFromDb = set(self.groupService.getAccesses(repository.groupName))
            ...
            for accessData in repository.accesses:
                ...
                for filter in filters:
                    for (url, method) in urlsMethods:
                            ...
                            try: 
                                self.groupService.remAcl(repository.groupName, accessId)
                                self.groupService.addAcl(repository.groupName, accessId)
                                if filter:
                                    self.groupService.registerFilter(repository.groupName, accessId, filter)
